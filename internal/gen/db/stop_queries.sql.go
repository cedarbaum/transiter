...
SELECT trip_stop_time.pk, trip_stop_time.stop_pk, trip_stop_time.trip_pk, trip_stop_time.arrival_time, trip_stop_time.arrival_delay, trip_stop_time.arrival_uncertainty, trip_stop_time.departure_time, trip_stop_time.departure_delay, trip_stop_time.departure_uncertainty, trip_stop_time.stop_sequence, trip_stop_time.track, trip_stop_time.headsign, trip_stop_time.past,
       trip.pk, trip.id, trip.route_pk, trip.direction_id, trip.started_at, trip.gtfs_hash, trip.feed_pk,
...
LEFT JOIN vehicle ON vehicle.trip_id = trip.id AND vehicle.system_pk = $1
WHERE trip_stop_time.stop_pk = ANY($2::bigint[])
AND NOT trip_stop_time.past
ORDER BY trip_stop_time.departure_time, trip_stop_time.arrival_time
`

type ListTripStopTimesByStopsParams struct {
	SystemPk int64
	StopPks  []int64
}
...
func (q *Queries) ListTripStopTimesByStops(ctx context.Context, arg ListTripStopTimesByStopsParams) ([]ListTripStopTimesByStopsRow, error) {
	rows, err := q.db.Query(ctx, listTripStopTimesByStops, arg.SystemPk, arg.StopPks)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
