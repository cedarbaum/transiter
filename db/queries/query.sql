...
-- name: ListStopsInSystemGeo_ByDistance :many
SELECT * FROM stop
WHERE system_pk = sqlc.arg(system_pk)
  AND (
    NOT sqlc.arg(only_return_specified_ids)::bool OR
...
ORDER BY (6371 * acos(cos(radians(latitude)) * cos(radians(sqlc.arg(latitude)::numeric)) * cos(radians(sqlc.arg(longitude)::numeric) - radians(longitude)) + sin(radians(latitude)) * sin(radians(sqlc.arg(latitude)::numeric))));

-- name: ListStopsInSystemGeo_ById :many
SELECT * FROM stop
WHERE system_pk = sqlc.arg(system_pk)
  AND (
    NOT sqlc.arg(only_return_specified_ids)::bool OR
