...
const listStopsInSystemGeo_ByDistance = `-- name: ListStopsInSystemGeo_ByDistance :many
SELECT pk, id, system_pk, source_pk, parent_stop_pk, name, longitude, latitude, url, code, description, platform_code, timezone, type, wheelchair_boarding, zone_id FROM stop
WHERE system_pk = $1
  AND (
    NOT $2::bool OR
...
ORDER BY (6371 * acos(cos(radians(latitude)) * cos(radians($4::numeric)) * cos(radians($5::numeric) - radians(longitude)) + sin(radians(latitude)) * sin(radians($4::numeric))))
`

type ListStopsInSystemGeo_ByDistanceParams struct {
	SystemPk               int64
	OnlyReturnSpecifiedIds bool
	StopIds                []string
...
func (q *Queries) ListStopsInSystemGeo_ByDistance(ctx context.Context, arg ListStopsInSystemGeo_ByDistanceParams) ([]Stop, error) {
	rows, err := q.db.Query(ctx, listStopsInSystemGeo_ByDistance,
		arg.SystemPk,
		arg.OnlyReturnSpecifiedIds,
		arg.StopIds,
		arg.Latitude,
...
const listStopsInSystemGeo_ById = `-- name: ListStopsInSystemGeo_ById :many
SELECT pk, id, system_pk, source_pk, parent_stop_pk, name, longitude, latitude, url, code, description, platform_code, timezone, type, wheelchair_boarding, zone_id FROM stop
WHERE system_pk = $1
  AND (
    NOT $2::bool OR
...
type ListStopsInSystemGeo_ByIdParams struct {
	SystemPk               int64
	OnlyReturnSpecifiedIds bool
	StopIds                []string
	Latitude               pgtype.Numeric
...
func (q *Queries) ListStopsInSystemGeo_ById(ctx context.Context, arg ListStopsInSystemGeo_ByIdParams) ([]Stop, error) {
	rows, err := q.db.Query(ctx, listStopsInSystemGeo_ById,
		arg.SystemPk,
		arg.OnlyReturnSpecifiedIds,
		arg.StopIds,
		arg.Latitude,
