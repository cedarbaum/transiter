...
func updateVehicles(ctx context.Context, updateCtx common.UpdateContext, vehicles []gtfs.Vehicle) error {
	...
	for _, vehicle := range vehicles {
		...
		// TODO: We can insert a vehicle with no ID if it has an associated trip.
	...
	if len(validVehicleEntities) == 0 {
		return updateCtx.Querier.ClearVehicleFeed(ctx, updateCtx.FeedPk)
	}
	...
	vehicleIDToRow, tripIDToRow, err := dbwrappers.MapVehicleIDAndTripIDToRow(ctx, updateCtx.Querier, updateCtx.SystemPk, vehicleIDs)
	if err != nil {
		return err
	}
	...
	for _, vehicle := range validVehicleEntities {
		...
		var tripIDOrNil *string = nil
		if vehicle.GetTrip().ID.ID != "" {
			if vehicleData, ok := tripIDToRow[vehicle.GetTrip().ID.ID]; ok && vehicleData.ID.String != *vehicle.ID.ID {
				updateCtx.Logger.DebugCtx(ctx, "Trip ID already used by another vehicle", vehicle.GetTrip().ID.ID)
				continue
			}
			insertedTripIDs[vehicle.GetTrip().ID.ID] = true
			tripIDOrNil = ptr(vehicle.GetTrip().ID.ID)
		}

		var stopPk *int64 = nil
		if vehicle.StopID != nil {
		...
		if existingVehicleDataForID, vehicleExists := vehicleIDToRow[*vehicle.ID.ID]; vehicleExists {
			// Update
			err := updateCtx.Querier.UpdateVehicle(ctx, db.UpdateVehicleParams{
				Pk:                  existingVehicleDataForID.Pk,
				FeedPk:              updateCtx.FeedPk,
				CurrentStopPk:       convert.NullInt64(stopPk),
				TripID:              convert.NullString(tripIDOrNil),
